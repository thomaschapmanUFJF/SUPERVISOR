...
import struct
import random
from .schema import PAYLOAD_FORMAT, FRAME_SCHEMA
...
def calculate_frame_crc(frame):
    ...
    return calculate_crc(type, length, payload)


def crc_ok(frame):
    crc_value = calculate_frame_crc(frame)
...
def build_frame(payload, type=0x01):
    ...
    return bytearray(
        [SYNC_BYTE_1, SYNC_BYTE_2, type, length, *payload, crc_bytes[0], crc_bytes[1]]
    )
